...
//         return NULL;
//     }
//     R_SCI_UART_Read(instance->p_ctrl, &instance->temp_rx_byte, 1);
...
// {
//     if (_instance->tx_busy)
//         return; // 忙则跳过

//     _instance->tx_busy = true;

//     /*
//      * 如果配了 DTC for Transmission，
//      * 这个 Write 函数会自动启动 TX DTC 进行搬运，CPU 直接返回。
//      */
//     R_SCI_UART_Write(_instance->p_ctrl, send_buf, send_size);
// }

// /* API：从 RingBuffer 取数据 (给上层调用) */
// uint16_t USART_Read_RingBuffer(USARTInstance *_instance, uint8_t *buff, uint16_t len)
// {
//     if (!_instance || !buff || len == 0)
//         return 0;
//     uint16_t count = 0;
//     while (count < len && _instance->tail != _instance->head)
//     {
//         buff[count++] = _instance->rx_ring_buffer[_instance->tail];
//         _instance->tail = (_instance->tail + 1) % UART_RING_BUFFER_SIZE;
//     }
//     return count;
// }
...
// {
//     return !_instance->tx_busy;
// }

// /**
//  * @brief 全局 UART 回调处理
//  *        在 hal_entry.c 的 user_uart0_callback 中调用此函数
//  */
// void UART_Global_Callback(uart_callback_args_t *p_args)
// {
//     USARTInstance *target = NULL;
...
//         p_args->event == UART_EVENT_BREAK_DETECT)
//     {
//         /* 这里的 Read 实际上起到了清除 Error Flag 并重启接收的作用 */
//         R_SCI_UART_Read(target->p_ctrl, &target->temp_rx_byte, 1);
//         return;
//     }
//     else if (p_args->event == UART_EVENT_RX_COMPLETE)
//     {
//         // 1. 取数据 (DTC搬完了，或者1字节读完了，数据在temp里)
//         uint8_t rx_data = target->temp_rx_byte;
        
//         // 2. 入队
//         uint16_t next_head = (target->head + 1) % UART_RING_BUFFER_SIZE;
//         if (next_head != target->tail) {
//             target->rx_ring_buffer[target->head] = rx_data;
//             target->head = next_head;
//         }
        
//         // 3. 续订 (这是最关键的一步，放在最后或者取完数据后)
//         fsp_err_t err = R_SCI_UART_Read(target->p_ctrl, &target->temp_rx_byte, 1);
//         if (FSP_SUCCESS != err)
//         {
//             while (1);
//         }
