...
// 角度制转弧度制: rad = deg * (PI / 180)
#define DEG_TO_RAD(angle) ((angle) * 0.017453292519943295769f)

// 弧度制转角度制: deg = rad * (180 / PI)
#define RAD_TO_DEG(angle) ((angle) * 57.295779513082320876f)

// 圈数转弧度: rad = turns * 2 * PI
#define TURNS_TO_RAD(turns) ((turns) * 2.0f * _PI)

// 弧度转圈数: turns = rad / (2 * PI)
#define RAD_TO_TURNS(rad) ((rad) * 0.15915494309189533576f)

// 圈数转角度: deg = turns * 360
#define TURNS_TO_DEG(turns) ((turns) * 360.0f)

// 角度转圈数: turns = deg / 360
#define DEG_TO_TURNS(deg) ((deg) * 0.00277777777777777778f)

/* 兼容性宏：使用标准库 math.h */
#define cosf_kb(x) cosf(x)
#define sinf_kb(x) sinf(x)
#define RAD_TO_DEG_CONST 57.2957795f
#define DEG_TO_RAD_CONST 0.01745329f
