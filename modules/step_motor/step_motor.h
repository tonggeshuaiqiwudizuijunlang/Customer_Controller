...
typedef struct
{
    ...
    bool is_finished;           // 运动是否完成 (from 0x23 byte[4])
} Joint_t;
...
/**
 * @brief 位置控制 (最快速度到达)
 * @param need_ack 是否需要电机回复当前位置 (cmd 0x23)
 */
 ...
 * @param turns 目标位置 (圈)
 * @param speed_limit 最大巡航速度 (圈/秒)
 */
void Joint_Motor_Set_Pos_With_SpeedLimit(uint8_t id, float turns, float speed_limit);
